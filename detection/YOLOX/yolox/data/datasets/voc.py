...
class VOCDetection(Dataset):
    ...
    def __init__(
        ...
        self.standard_voc = standard_voc
        ...
        if standard_voc:
            ...
            for (year, name) in image_sets:
                self._year = year
                rootpath = os.path.join(self.root, name+year)
                label_files = os.listdir(os.path.join(rootpath,"Annotations"))
                self.ids.extend((rootpath, i.split('.')[0]) for i in label_files if i.endswith('.xml'))
    ...
    def _do_python_eval(self, output_dir="output", iou=0.5):
        if self.standard_voc:
            ...
            name = self.image_set[0][1]
            rootpath = os.path.join(self.root, name+self._year)
            annopath = os.path.join(rootpath, "Annotations", "{}.xml")
            imagesetfile = [i[1] for i in self.ids]
            cachedir = os.path.join(
                self.root, "annotations_cache", "VOC" + self._year, name
            )
