...
if __name__ == "__main__":
    """
        在测试的时候需要注意的是：  CLASSES需要导入训练时候对应的类
            # predictor = Predictor(model, exp, COCO_CLASSES, trt_file, decoder, args.device, args.fp16, args.legacy)
            predictor = Predictor(model, exp, VOC_CLASSES, trt_file, decoder, args.device, args.fp16, args.legacy)
            
        测试命令：
        python tools/demo.py image -f exps/example/yolox_voc/yolox_voc_s.py -c YOLOX_outputs/yolox_voc_s/best_ckpt.pth --path datasets/xx.jpg --conf 0.25 --nms 0.45 --tsize 640 --save_result --device gpu
    """

    # python tools/demo.py image -f exps/example/yolox_voc/yolox_voc_s.py -c YOLOX_outputs/yolox_voc_s/best_ckpt.pth --path datasets/voc/test2007/JPEGImages/road9.png --conf 0.25 --nms 0.45 --tsize 640 --save_result --device gpu
