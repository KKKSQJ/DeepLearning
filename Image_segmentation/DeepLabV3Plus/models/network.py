...
from models.deeplabv3plus import deeplabv3plus_resnet50, deeplabv3plus_resnet101, deeplabv3plus_mobilenetv3_large
...
MODEL = {
    ...
    "deeplabv3plus_mobilenetv3_large": deeplabv3plus_mobilenetv3_large
}
...
def build_model(cfg='config/example.yaml', num_classes=21, pretrain=True):
    ...
    model = MODEL[model_name](aux, num_classes, output_stride, pretrain_backbone, pretrain_weights)
    ...
    if pretrain:
        ...
        if "model_state" in weights_dict.keys():
            weights_dict = weights_dict["model_state"]

        if num_classes != 21:
            # 官方提供的预训练权重是21类(包括背景)
            # 如果训练自己的数据集，将和类别相关的权重删除，防止权重shape不一致报错
            for k in list(weights_dict.keys()):
                if "classifier.3" in k or "classifier.4" in k:
                    del weights_dict[k]
