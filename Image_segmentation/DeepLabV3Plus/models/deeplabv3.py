...
class DeepLabHead(nn.Sequential):
    def __init__(self, in_channels, out_channels, atrous_rate: List[int] = [12, 24, 36]):
        super(DeepLabHead, self).__init__(
            ASPP(in_channels, atrous_rate, 256),
...
def deeplabv3_resnet50(aux, num_classes=21, output_stride=8, pretrain_backbone=False, pretrain_weights=None):
    ...
    classifier = DeepLabHead(out_inplanes, num_classes, aspp_dilate)

    model = DeepLabv3(backbone, classifier, aux_classifier)
    return model


def deeplabv3_resnet101(aux, num_classes=21, output_stride=8, pretrain_backbone=False, pretrain_weights=None):
...
def deeplabv3_mobilenetv3_large(aux, num_classes=21, output_stride=8, pretrain_backbone=False, pretrain_weights=None):
    ...
    if output_stride == 8:
        ...
        backbone = mobilenet_v3_large(dilated=False)
        aspp_dilate = [6, 12, 18]
...
if __name__ == '__main__':
    ...
    model = deeplabv3_mobilenetv3_large(aux=True, num_classes=21, output_stride=16, pretrain_backbone=False)
    y = model(x)
    print(y["out"].shape)
