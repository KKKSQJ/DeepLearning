...
class AffineTransform(object):
    ...
    def __init__(self,
                 ...
                 fixed_size: Tuple[int, int] = (256, 192)):  # h w
        self.scale = scale
    ...
    def __call__(self, img, target):
        if "box" not in target:
            target["box"] = [0, 0, target["image_width"], target["image_height"]]
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, target):
        if random.random() < self.p:
            ...
            if self.matched_parts is not None:
...
class KeypointToHeatMap(object):
    def __init__(self,
        ...
        self.keypoints_nums = keypoints_nums
        self.down_ratio = 4
    ...
    def __call__(self, image, target):
        ...
        heatmap = np.zeros((self.keypoints_nums, self.heatmap_hw[0], self.heatmap_hw[1]), dtype=np.float32)
        # heatmap_kps = (kps / self.down_ratio + 0.5).astype(np.int)  # round
        heatmap_kps = (kps / self.down_ratio).astype(np.int)  # round
        for kp_id in range(num_kps):
            label = int(labels[kp_id])
            v = kps_weights[kp_id]
            if v < 0.5:
                # 如果该点的可见度很低，则直接忽略
                continue
            ...
            if kps_weights[kp_id] > 0.5:
                ...
                kernel_value = torch.max(torch.from_numpy(heatmap[label][img_y[0]:img_y[1] + 1, img_x[0]:img_x[1] + 1]),
                                         torch.from_numpy(self.kernel[g_y[0]:g_y[1] + 1, g_x[0]:g_x[1] + 1])).numpy()

                heatmap[label][img_y[0]:img_y[1] + 1, img_x[0]:img_x[1] + 1] = kernel_value
        ...
        # 计算点的偏移量
        max_objs = 100  # 每张图上最多点个数
        regs = np.zeros((max_objs, 2), dtype=np.float32)  # 每个点偏移量
        inds = np.zeros((max_objs,), dtype=np.int64)
        ind_masks = np.zeros((max_objs,), dtype=np.uint8)  # 点的mask

        for k, kp in enumerate(kps):
            if kp[0]<0 or kp[1] <0 or kp[0] >= self.heatmap_hw[0]*self.down_ratio or kp[1] >=self.heatmap_hw[1]*self.down_ratio:
                # print(kp)
                continue
            kp = kp / self.down_ratio

            kp_int = kp.astype(np.int32)
            regs[k] = kp - kp_int
            inds[k] = kp_int[1] * self.heatmap_hw[1] + kp_int[0]
            ind_masks[k] = 1
        target['max_objs'] = torch.as_tensor(max_objs, dtype=torch.uint8)
        target['regs'] = torch.as_tensor(regs, dtype=torch.float32)
        target['inds'] = torch.as_tensor(inds, dtype=torch.int64)
        target['inds_masks'] = torch.as_tensor(ind_masks, dtype=torch.uint8)
