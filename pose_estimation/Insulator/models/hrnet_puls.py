...
class HighResolution(nn.Module):
    def __init__(self, base_channel: int = 32, num_joint: int = 17, use_offset=False, stage_block=None):
        ...
        self.regs = nn.Conv2d(base_channel, 2, kernel_size=1, stride=1, padding=0)
        ...
        for m in self.named_children():
            if m[0] in "regs":
                if isinstance(m[1], nn.Conv2d):
                    nn.init.normal_(m[1].weight, std=0.001)
                    nn.init.constant_(m[1].bias, 0)
            # elif m[0] in "final_layer":
            #     if isinstance(m[1], nn.Conv2d):
            #         nn.init.constant_(m[1].bias, -2.19)
...
if __name__ == '__main__':
    model = HighResolution(base_channel=32, num_joint=1, use_offset=True)
    print(model)

    x = torch.rand(2, 3, 224, 224)
    heatmap, offset = model(x)
    print(heatmap)
    print(offset)
